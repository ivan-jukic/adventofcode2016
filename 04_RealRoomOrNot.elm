...
view model =
    let
        ...
        idsSum =
            List.foldl (\( h, id ) c -> c + id ) 0 realRooms

        decodedNames =
            realRooms
                |> List.filter ( \( h, i ) -> i > 0 )
                |> List.map ( \( h, i ) -> decodeName h i )
    in
        div []
            [ div [] [ text ( "Sector ID sum of real rooms: " ++ ( toString idsSum ) ) ]
            , div []
                [ h3 [] [ text "Decoded room names" ]
                , ul []
                    ( List.map
                        ( \( n, i ) ->
                            li [] [ text ( n ++ " " ++ ( toString i ) ) ]
                        ) decodedNames
                    )
                ]
            ]
...
isRealRoom: String -> ( String, Int )
isRealRoom roomHash =
    let
        parts =
            roomHash |> String.split "-"
...
getChecksum letters =
    let
        ...
        dictUpdate = 
            ( \maybeCount ->
                Just ( ( maybeCount |> Maybe.withDefault 0 ) + 1 )
            )
        ...
        sortedCounts =
            letters
                |> String.toList 
                |> List.foldl
                    ( \l d -> Dict.update l dictUpdate d ) Dict.empty
                |> Dict.toList
                |> List.sortWith
                    ( \( l1, c1 ) ( l2, c2 ) ->
                        let
                            code1 = Char.toCode l1
                            code2 = Char.toCode l2
                        in
                            case compare c1 c2 of
                                LT -> GT
                                EQ ->
                                    case compare code1 code2 of
                                        LT -> LT
                                        EQ -> EQ
                                        GT -> GT
                                GT -> LT
...
decodeName: String -> Int -> ( String, Int )
decodeName hash id =
    let
        codeMax =
            122
