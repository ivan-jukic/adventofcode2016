    ...
    let
       ( tlsCount, sslCount ) =
            mainInput
                |> String.lines
                |> List.map ( \addr ->
                    ( isValidTlsString addr 0 False False
                    , isValidSslString addr
                    )
                )
                |> List.unzip
    in
        div []
            [ div [] [ text ( "IPv7 TLS addresses: " ++ ( toString ( List.sum tlsCount ) ) ) ]
            , div [] [ text ( "IPv7 SSL addresses: " ++ ( toString ( List.sum sslCount ) ) ) ]
            ]


isValidTlsString: String -> Int -> Bool -> Bool -> Int
isValidTlsString address idx abba inBrackets =
    if ( String.length address ) - 4 >= idx then
        let
            newIdx = idx + 1

            currentChar = address |> String.dropLeft idx |> String.left 1
        in
            if currentChar == "[" then
                isValidTlsString address newIdx abba True

            else if currentChar == "]" then
                isValidTlsString address newIdx abba False

            else if abba && not inBrackets then
                isValidTlsString address newIdx abba inBrackets

            else
                let
                    chars =
                        address |> String.dropLeft idx |> String.left 4

                    ( a, b, c, d ) =
                        ( chars |> String.left 1
                        , chars |> String.dropLeft 1 |> String.left 1
                        , chars |> String.dropLeft 2 |> String.left 1
                        , chars |> String.dropLeft 3
                        )

                    isAbba =
                        a == d && b == c && a /= b
                in
                    if inBrackets && isAbba then
                        0
                    else
                        isValidTlsString address newIdx ( if abba then abba else isAbba ) inBrackets
    else
        if abba then 1 else 0


isValidSslString: String -> Int
isValidSslString address =
    let
        lookingForYXY = 
            getAllAbaSubstrings address 0 False

        hypernetSequences =
            address
                |> split Regex.All ( regex "\\[|\\]" )
                |> List.indexedMap (,)
                |> List.filter ( \(i, s) -> i % 2 == 1 )
                |> List.map ( \(i, s) -> s )
                |> String.join "-"
        ...
        doesSplitAddress =
            lookingForYXY
                |> List.map ( \yxy ->
                    let
                        parts =
                            String.split yxy hypernetSequences
                    in
                        List.length parts > 1
                )
    in
        if ( List.any (\b -> b) doesSplitAddress ) then 1 else 0 


getAllAbaSubstrings: String -> Int -> Bool -> List String
getAllAbaSubstrings address idx inBrackets =
    if ( String.length address ) - 3 >= idx then
        let
            newIdx =
                idx + 1

            ( firstChar, lastChar ) =
                ( address |> String.dropLeft idx |> String.left 1
                , address |> String.dropLeft ( idx + 2 ) |> String.left 1
                )
            ...
            newInBrackets =
                if lastChar == "[" then
                    ...
                    inBrackets
        in
            if inBrackets || newInBrackets then
                getAllAbaSubstrings address newIdx newInBrackets

            else
                let
                    prevFound =
                        getAllAbaSubstrings address newIdx inBrackets

                    xyz =
                        address |> String.dropLeft idx |> String.left 3

                    ( x, y, z ) =
                        ( xyz |> String.left 1
                        , xyz |> String.dropLeft 1 |> String.left 1
                        , xyz |> String.dropLeft 2
                        )
                in
                    if x == z && x /= y then
                        let
                            yxy =
                                y ++ x ++ y
                        in
                            if ( List.member yxy prevFound ) then
                                prevFound
                            else
                                List.append [ yxy ] prevFound
                    else
                        prevFound
    else
        []
...
{--
 -- Old solution for first task
tlsAddress: String -> Int
tlsAddress address =
    let
        ( nonBracket, bracket ) =
            address
                |> split Regex.All ( regex "\\[|\\]" )
                |> List.map stringHasABBA
                |> List.indexedMap (,)
                |> List.partition ( \( i, v ) -> i % 2 == 0 )
    ...
    in
        if abba && noAbba then 1 else 0


stringHasABBA: String -> Bool
stringHasABBA addressPart =
    List.range 0 ( ( String.length addressPart ) - 4 )
        |> List.foldl
            ( \idx curr ->
                if not curr then
                    let
                        chars =
                            addressPart |> String.dropLeft idx |> String.left 4
