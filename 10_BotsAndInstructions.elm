...
import Regex exposing ( find, replace, regex, escape, contains )
...
type alias Model =
    { assign: List Command
    , pass: Dict Int PassData
    , output: Dict Int ( List Int )
    , searchLo: Int
    , searchHi: Int
    , searchOutput: List Int
    }
...
init =
    let
        ( assignCmds, passCmds ) = splitCommands mainInput

        assign = assignCmds
            |> List.map ( \cmd ->
                case ( find Regex.All assignRgx cmd ) of
                    m :: [] ->
                        m |> parseAssignCmd
                    _ ->
                        Unknown
        ...
        pass = passCmds
            |> List.foldl ( \cmd dict ->
                case ( find Regex.All passRgx cmd ) of
                    m :: [] ->
                        case ( m |> parsePassCmd ) of
                            Just ( bot, pass ) ->
                                dict |> Dict.insert bot pass
                            Nothing ->
                                dict
                    _ ->
                        dict
    ...
    in
        ( Model assign pass Dict.empty 17 61 [ 0, 1, 2 ]
...
update msg model =
    case msg of
        ...
        NextCommand ->
            case model.assign of
                cmd :: _ ->
                    let
                        ( newModel, action ) =
                            processAssignCmd cmd model
                    in
                        ( { newModel | assign = List.drop 1 model.assign }
                        , Cmd.batch [ action, Task.perform ( \_ -> NextCommand ) ( Task.succeed () ) ]
                        )

                [] ->
                    ( { model | assign = List.drop 1 model.assign }, Cmd.none )

        PassChips data ->
            let
                ( loModel, loCmd ) =
                    processPassCmd data.loCmd data.lo model

                ( hiModel, hiCmd ) =
                    processPassCmd data.hiCmd data.hi loModel
            in
                ( hiModel, Cmd.batch [ loCmd, hiCmd ] )


processPassCmd: Command -> Maybe Int -> Model -> ( Model, Cmd Msg )
processPassCmd cmd maybeVal model =
    case maybeVal of
        Just val ->
            case cmd of
                PassToBot bot ->
                    updateModelWithNewData cmd bot val model

                PassToOutput out ->
                    case ( Dict.get out model.output ) of
                        Just output ->
                            let
                                newOutput =
                                    output ++ [ val ]
                                ...
                                newOutputDict =
                                    Dict.insert out newOutput model.output
                            in
                                ( { model | output = newOutputDict }, Cmd.none )

                        Nothing ->
                            ( { model | output = ( Dict.insert out [ val ] model.output ) }, Cmd.none )
                _ ->
                    ( model, Cmd.none )
        Nothing ->
            ( model, Cmd.none )
...
processAssignCmd: Command -> Model -> ( Model, Cmd Msg )
processAssignCmd cmd model =
    case cmd of
        Assign bot val ->
            updateModelWithNewData cmd bot val model
...
updateModelWithNewData: Command -> Int -> Int -> Model -> ( Model, Cmd Msg )
updateModelWithNewData cmd bot val model =
    case ( Dict.get bot model.pass ) of
        Just pass ->
            let
                ( newLo, newHi, passOn ) =
                    getNewLoHi val pass.lo pass.hi

                newPassData =
                    { pass | lo = newLo, hi = newHi }

                newPass =
                    Dict.insert bot newPassData model.pass

                action =
                    case passOn of
                        True ->
                            Task.perform ( \a -> PassChips a ) ( Task.succeed newPassData )
                        False ->
                            Cmd.none

                --d = Debug.log "bot updated" ( cmd, bot, newLo, newHi )
            in
                ( { model | pass = newPass }, action )
...
getNewLoHi: Int -> Maybe Int -> Maybe Int -> ( Maybe Int, Maybe Int, Bool )
getNewLoHi val maybeLo maybeHi =
    case ( maybeLo, maybeHi ) of

        ( Nothing, Nothing ) ->
            ( Just val , Nothing, False )

        ( Just v, Nothing ) ->
            if v > val then
                ( Just val, Just v, True )
            else
                ( Just v, Just val, True )
    ...
    let
        bot = model.pass
            |> Dict.filter ( \bot p ->
                case ( p.hi, p.lo ) of
                    ( Just hi, Just lo ) ->
                        lo == model.searchLo && hi == model.searchHi
                    _ ->
                        False
            )
            |> Dict.keys
        ...
        prod = model.output
            |> Dict.filter ( \out v -> List.member out model.searchOutput )
            |> Dict.toList
            |> List.map ( \( out, arrVal ) -> List.product arrVal )
            |> List.product
        
    in
        div [ style [ ( "padding", "20px" ) ] ]
            [ div [] 
                    [ text ( "Bot with chips " ++ ( toString model.searchLo ) ++ " and " ++ ( toString model.searchHi ++ ": " ++  bot ) ) ]
            , div []
                [ text ( "Product of output " ++ ( toString model.searchOutput ) ++ ": " ++ ( toString prod ) ) ]
            ]


assignRgx: Regex.Regex
assignRgx =
    regex "^value\\s+(\\d+)\\s+goes\\s+to\\s+bot\\s+(\\d+)$"


passRgx: Regex.Regex
passRgx =
    regex "^bot\\s+(\\d+)\\s+gives\\s+(low)\\s+to\\s+(bot|output)\\s+(\\d+)\\s+and\\s+(high)\\s+to\\s+(bot|output)\\s+(\\d+)$"


splitCommands: String -> ( List String, List String )
splitCommands input =
    input
        |> String.lines
        |> List.partition ( \i -> contains assignRgx i )   


parseAssignCmd: Regex.Match -> Command
parseAssignCmd match =
    case match.submatches of
        v :: b :: [] ->
            Assign ( b |> maybeStringToInt ) ( v |> maybeStringToInt )
        _ ->
            Unknown


parsePassCmd: Regex.Match -> Maybe ( Int, PassData )
parsePassCmd match =
    case match.submatches of
        botN :: _ :: lowDest :: lowNum :: _ :: hiDest :: hiNum :: [] ->
            Just
                ( botN |> maybeStringToInt
                , PassData 
                    Nothing
                    Nothing
                    ( case lowDest of
                        Just "bot" ->
                            PassToBot ( lowNum |> maybeStringToInt )
                        _ ->
                            PassToOutput ( lowNum |> maybeStringToInt )
                    )
                    ( case hiDest of
                        Just "bot" ->
                            PassToBot ( hiNum |> maybeStringToInt )
                        _ ->
                            PassToOutput ( hiNum |> maybeStringToInt )
                    )
                )
        _ ->
            Nothing
...
maybeStringToInt ms =
    ms |> withDefault "0" |> toInt |> Result.withDefault 0
