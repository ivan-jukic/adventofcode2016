...
import Regex exposing (replace, regex, escape)
...
view model =
    let
        decompressedV1String =
            decompressV1 mainInput ""

        v2Length =
            getLength 1 0 mainInput
    in
        div []
            [ div [] [ text ( "Decompressed V1 length is: " ++ ( toString ( String.length decompressedV1String ) ) ) ]
            , div [] [ text ( "Decompressed V2 length is: " ++ ( toString v2Length ) ) ]
            ]


getLength: Int -> Int -> String -> Int
getLength multiplier total input =
    case input of
        "" ->
            total
        _ ->
            case ( getAxBMatch input ) of
                Just m ->
                    let
                        ( len, times ) =
                            getAxB m.submatches
                        ...
                        subInput =
                            input |> String.dropLeft matchLen |> String.left len

                        otherInput =
                            input |> String.dropLeft ( matchLen + len )

                        subInputTotal =
                            case ( subInput |> isOnlyLetters ) of
                                True ->
                                    len * times * multiplier
                                False ->
                                    getLength ( multiplier * times ) 0 subInput

                        otherInputTotal =
                            getLength multiplier 0 otherInput
                    in
                        total + subInputTotal + otherInputTotal

                Nothing ->
                    getLength multiplier ( total + 1 ) ( input |> String.dropLeft 1 )
            ...
            case ( getAxBMatch input ) of
                Just m ->
                    let
                        ...
                        repeatString =
                            input |> String.dropLeft ( String.length m.match ) |> String.left a
                    in
                        List.range 1 b
                            |> List.foldl ( \i c -> c ++ repeatString ) ""
                            |> String.append output
                            |> decompressV1
                                ( input |> String.dropLeft ( (String.length m.match ) + a ) )
                Nothing ->
                    output ++ ( String.left 1 input )
                        |> decompressV1 ( String.dropLeft 1 input )


getAxBMatch: String -> Maybe Regex.Match
getAxBMatch input =
    let
        rgx =
            Regex.regex "^\\((\\d+)x(\\d+)\\)"
    in
        case ( Regex.find ( Regex.AtMost 1 ) rgx input ) of
            m :: [] ->
                Just m
            _ ->
                Nothing
...
getAxB: List ( Maybe String ) -> ( Int, Int )
getAxB submatch =
    case submatch of
        [ a, b ] ->
            ( a |> maybeStringToInt
...
mainInput =
    {--
     -- test decompression result: ADVENTABBBBBCXYZXYZXYZABCBCDEFEFG(1x3)AX(3x3)ABC(3x3)ABCY
     -- }
    """ADVENTA(1x5)BC(3x3)XYZA(2x2)BCD(2x2)EFG(6x1)(1x3)AX(8x2)(3x3)ABCY"""
    --}
    {--}
    --"""(21x2)(2x5)AB(9x3)(4x2)CDEF"""
    --"""(3x3)XYZ"""
    --"""X(8x2)(3x3)ABCY"""
